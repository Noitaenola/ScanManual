...
% ------------------------------------------------------------------------------
	...
	\begin{figure}
		\subbottom[ScanSnap SV600, de \href{http://www.fujitsu.com/es/products/computing/peripheral/scanners/scansnap/sv600/}{Fujitsu}]{\includegraphics[width=.44\linewidth]{scannerScanSnap}}\hfill
		\subbottom[ET16, de \href{https://www.czur.com/product/et16/}{CZUR}]{\includegraphics[width=.54\linewidth]{scannerET16}}
	...
	\begin{figure}
		\hspace*{\fill}
		\subbottom[Workforce DS-410, de \href{https://www.epson.es/products/scanners}{Epson}]{\includegraphics[width=.4\linewidth]{scannerWorkforce}}\hfill
		\subbottom[ScanJet Pro 2500, de \href{https://store.hp.com/SpainStore/Merch/Product.aspx?id=L2747A&opt=B19&sel=SCN}{HP}]{\includegraphics[width=.45\linewidth]{scannerScanJet}}%
		\hspace*{\fill}
...
% ------------------------------------------------------------------------------
	...
	\note{Nota: páginas atípicas} Llamaremos \emph{páginas atípicas} a las correspondientes a las tapas y lomo, así como aquellas páginas que por alguna razón tengan un tamaño físico mayor o menor que el resto, como puede ocurrir con libros que traen páginas dobladas que pueden extenderse.

	\todo{Agregar a la historia que se cambió la definición de páginas atípicas y se ajustaron los proc. de los pasos 4-6 para reflejarlo.}
	...
	Para ayudarnos en esta tarea, también podemos ordenar las páginas: \menu{Order by increasing > width, height, deviation} nos permite agrupar todas las páginas más altas (bajas), más anchas (angostas) o que se desvían más (menos) de la media juntas. En esos conjuntos es más probable encontrar errores. Por ejemplo, en la figura~\ref{fig:STselectContentError} vemos que la página en cuestión era de las más altas no porque el contenido lo fuera, sino porque no había sido correctamente reconocido y debemos arreglarlo.
	...
	Todos los errores de selección de contenido que no corrijamos seguramente se hagan evidentes al compaginar el documento, sobre todo si somos escrupulosos con los márgenes. En ese punto serán aún fáciles de corregir, pero cuanto menos trabajo nos quede hacia adelante, mejor.
	...
	Para las páginas atípicas probablemente no usemos márgenes, por lo que desactivamos \menu{\typesetBox{Match size with other pages}} en esas páginas. Veremos que al hacerlo las dimensiones finales de estas páginas no afectarán a todas las demás.

	De momento concentrémonos en las típicas. Elijamos ordenar las páginas por alto (\menu{Order by increasing height}) y busquemos abajo del todo la típica más alta. En nuestro caso al final aparecen las tapas (obviamente) y páginas con tablas. La típica más alta es la p.~296. Vamos entonces a esa página en el libro y medimos los márgenes superior e inferior con la regla. Ingresamos esos valores y presionamos \menu{\keys{Apply To\...} > All pages}.

	Ahora pasamos a ordenar por ancho: \menu{Order by increasing width}. Haremos lo mismo que antes, buscaremos la típica más ancha par e impar. Son las pp.~250 y 117. Medimos para cada una el margen izquierdo y derecho, los ingresamos y elegimos \menu{\keys{Apply To\...} > Every other page}, para cada una. Las medidas en este caso se presentan en el cuadro~\ref{tab:pageMargins}.\footnote{Las medidas dieron 20\,mm y 11\,mm para las pares, y 10\,mm y 19\,mm para las impares. Debido a tolerancias de impresión y/o encuadernado las páginas no son simétricas, por lo que las medidas de los márgenes tampoco lo serán exactamente. Para no complicarnos, haremos que en nuestro resultado sí lo sean, simetrizando estos valores.}
	...
	Pero por el momento elijamos una página cualquiera y vayamos al paso 6. Buscando en la carpeta \typesetFolder{out} podemos ver de que dimensiones quedó. En este primer intento obtuve un archivo de 3955\,pix por 5790\,pix. Aplicando la fórmula que se da al principio de la sección concluimos que medirá 16,7\,cm por 24,5\,cm, lo que en ambas dimensiones es un poco más grande que el original físico. Esto seguramente se deba a las páginas atípicas, para las que deberemos ajustar manualmente los márgenes.
	...
	Eligiendo \menu{Mixed} el software intentará adivinar las áreas donde se encuentran las imágenes. En la pestaña \menu{Picture Zones} se señalan titilando con color azul. Por defecto estas áreas pueden tener cualquier forma, pero en el submenú \menu{Picture Shape > Rectangular} (fig~\ref{fig:SToutputPicureZonesRectangular}) podemos forzar a que sea rectangular, que servirá en la mayoría de los casos.

	\begin{figure}
		\centering
		\includegraphics[width=\linewidth]{SToutputPicureZonesRectangular}
		\caption[Sexto paso: Output. Se seleccionó la pestaña Picture zones]{Sexto paso: Output. Se seleccionó la pestaña \menu{Picture zones}\label{fig:SToutputPicureZonesRectangular}}
	\end{figure}
	...
	Activar \menu{Options > Equalize illumination (Color)} me ha dado buenos resultados. Téngase en cuenta.
	...
	Esto nos permitirá procesar cada parte en sus particularidades de manera independiente, de forma de lograr un resultado óptimo. Si no se va a hacer, de todas formas elegir la opción no hace daño, pues en \typesetFolder{out} se sigue guardando la imagen con las dos componentes.
	...
	\begin{enumerate}[noitemsep]
		\item La más sencilla es tratarlos como imágenes. Procedemos exactamente como se explicó antes.
		\item Si de dispone de un poco más de tiempo, se pueden editar en el \gimp para generar un contraste mayor que permita una binarización más efectiva. Para ello se pueden usar filtros o manualmente redibujar los detalles que se quieran conservar.
		\item La tercera consiste en ajustar el algoritmo de binarización, como se explica a continuación. Tiene el inconveniente de que los cambios afectan a toda la página y no solo al dibujo de interés.
	\end{enumerate}
	...
	Las cubiertas con diseños más complejos o que incluyen imágenes no admiten el mismo tipo de edición, aunque muchas veces es posible mejorarlas ajustando brillo, contraste, etc. Si bien la tapa es la presentación del libro, el peso en el tamaño final no debería ser excesivo.
	...
	\chapter{\texorpdfstring{\textsc{Ocr}}{OCR} con \abby\label{sec:OCR}} Una vez que las páginas fueron limpiadas y ajustadas y las imágenes separadas, podemos proceder a realizar el reconocimiento de texto. Como paso previo, notemos que que si marcamos \menu{Picture zones} y elegimos la opción \menu{Split output}, tenemos en la carpeta \typesetFolder{out} las imágenes procesadas y dentro de la misma otras dos carpetas, como se explicó antes.
	...
	Se pueden dibujar tantas áreas como uno quiera. Para nuestro procedimiento usaremos solo las de texto y las de tablas. No será necesario marcar imágenes (aún si no las hubiéramos separado con \scantailor), pues elegimos la opción \menu{Text under the page image}, que preservará las imágenes originales e insertará el texto reconocido debajo de las mismas.
	...
	\subsubsection{Configuración} Para que el patrón entrenado se use hay que tener marcada la opción \menu{\typesetDotCircle{Use user patterns}} con \menu{\typesetCheckbox{Also use built-in patterns}}, de manera de que se puedan complementar (ver figura~\ref{fig:ABBYoptionsOCR}). Solo el patrón personalizado es mala idea: no funciona bien y además no se adapta a los diferentes tamaños y fuentes que pueda haber en el documento.
	...
	\chapter{Creación del \texorpdfstring{\pdf}{PDF}} Llegó el momento de crear nuestro documento \pdf final. Básicamente, debemos insertar las imágenes (si es que las separamos), agregar los marcadores, corregir las etiquetas de las páginas, incluir los metadatos, eventualmente optimizar el documento y guardar en formato \pdfa. Fácil, ¿no?.
	...
	\begin{itemize}[noitemsep]
		...
		\item \mintinline{monkey}|-quality 40| es el nivel de compresión (va de 1 a 100);\footnote{En este caso elegí un valor de 40, pero deberemos encontrar un valor que nos satisfaga en cada caso.} y
		\item \mintinline{monkey}|SCAN_XXX.tif.pdf| es el archivo \pdf de salida.
	\end{itemize}

	Luego superpone las imágenes sobre las páginas correspondientes. Para que esta superposición no opaque el resto del contenido es esencial la opción \mintinline[breaklines=true]{monkey}|-transparent white| de \imagemagick.\footnote{Recomiendo probar qué ocurre si se omite.} Cada uno difiere en la forma en la que insertan esas páginas en nuestro documento base.
	...
	Su ejecución puede tomar un par de minutos. Al terminar tendremos un archivo \typesetFile{SCAN\_INS.pdf} con las imágenes insertadas.
	...
	\begin{figure}
		...
		\caption{Consola de \emph{Adobe Javascript} lista para insertar imágenes\label{fig:pdfPhotosJavascriptConsole}}
	\end{figure}

	Luego pegamos el contenido de \typesetFile{pdfPhotos.js} en la consola, lo seleccionamos todo y presionamos \keys{\ctrl + \return}. Después de algunos segundos aparecerá nuestro archivo \typesetFile{SCAN\_INS.pdf} con las imágenes insertadas. Si \emph{no} ocurrió ningún error, la consola mostrará el mensaje \texttt{undefined}.

	\subsection{Nota: sobre el tamaño final} Descubrí que si bien este método funciona bien, el blanco (convertido en transparente) de los \jpeg en algunos casos puede llegar a duplicar el tamaño de las imágenes. De momento lo presento así, pero hay que idear un método más eficiente, que recorte las imágenes (fácil) y las inserte en la posición adecuada en el documento final (difícil). (Acepto sugerencias.)
	...
	\section{Optimización\label{sec:pdfOptimize}} A veces podemos optimizar el \pdf para reducir un poco el tamaño sin afectar demasiado la calidad. Para ello seleccionamos \menu{File > Save as Other > Optimized PDF\ldots} y elegimos las opciones que queramos (ver fig.~\ref{fig:pdfOptimizeImages}).
	...
	Yo prefiero usar el menú \menu{Tools > Preflight} y la pestaña \menu{Standards} (ver fig.~\ref{fig:pdfPdfaPreflightStandards}). Allí elegimos \menu{Save as PDF/A}, presionamos \keys{Continue\ldots}, elegimos el estándar (recomiendo \menu{PDF/A-2u} o \menu{PDF/A-2b}), presionamos \keys{Continue\ldots} nuevamente y por último cambiamos las opciones. En principio marcamos solo \menu{Apply corrections} y vemos que sucede.
	...
	Para utilizar con mis documentos, yo creo tres perfiles: \menu{MyBitonal}, \menu{MyScan} y \menu{MyPhoto}. Básicamente, \menu{MyBitonal} es para páginas bitonales, \menu{MyPhoto} para fotos y \menu{MyScan} para páginas mixtas, donde puede haber texto sobre imágenes. Fueron creados en base a los perfiles existentes, a la documentación de \dexpress y a la información del \emph{Scan\&Share Tutorial} \cite{ScanAndShare2008}.
	...
	Para comprimirlo abrimos el \djvusmall (ver figura~\ref{fig:djvuSmallBody}) y usamos \keys{Open File(s)} para agregar las imágenes del cuerpo. Seleccionamos luego el perfil \menu{MyBitonal} del menú desplegable \menu{Select encoding profile}. Elegimos el directorio de salida y presionamos \keys{Convert}. Al archivo resultante le llamaremos \typesetFile{BODY.djvu}.
	...
	Este usa una versión modificada de la aplicación \typesetExe{c44.exe} de \djvulibre\footnote{\typesetExe{fi\_c44.exe} utiliza la biblioteca libre \href{http://freeimage.sourceforge.net/}{\textsc{FreeImage}} para cargar archivos \tiff directamente. De lo contrario habría que convertir primero a \textsc{ppm}.} para codificar las imágenes, y luego las inserta en nuestro documento.
	...
	\begin{itemize}[noitemsep]
		...
		\item botones \keys{Insert in DjVu} (ídem \keys{View}), para insertar las imágenes, \keys{Options} para configurar algunas opciones y \keys{Help} para abrir el archivo de ayuda; y
	...
	Podemos ir apreciando el resultado de nuestras opciones en el archivo actual presionando \keys{Current}. Una vez hallamos decidido los parámetros para todos los archivos (o para cada archivo en particular si así lo deseamos) presionamos \keys{Convert} para convertirlos todos. Una vez que termina hacemos \keys{Insert in DjVu} y obtenemos nuestro archivo con las imágenes insertadas.
	...
	En la primera (fig.~\ref{fig:djvuSmallModTransformTab}) nos interesará solo activar \menu{\typesetCheckbox{Subsample} > 2}. Esta tiene el mismo efecto que la \menu{BSF} de la sección~\ref{sec:djvuImager}, es decir, reducir las dimensiones de las imágenes a la mitad. Recomiendo dejarla en 2 por las razones que allí se explicaron.
	...
	\begin{itemize}
		\item v1.1 -- 2020-XX-XX: 
			\begin{itemize}
				...
				\item Ampliada la sección~\ref{sec:STselectContent}.
...
%				\item Cambiada la licencia a CC BY-SA 4.0.
				\item Otras correcciones menores.
			\end{itemize}
...
\begin{itemize}
	...
	\item Otra referencia interesante (en ruso)L \href{http://publ.lib.ru/cgi/forum/YaBB.pl?catselect=ocr}{OCR Forum}.
	\item revisar la sección de optimización de \pdf; aclarar que ahora (\abby 14) quizás no sea necesaria.
	\item archivos ocultos en estructura de \abby.
	\item Arreglar el caption de \ref{fig:epsonScanBitonal}.
