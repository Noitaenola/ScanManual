...
% ------------------------------------------------------------------------------
	...
	\pythonScript{pdfPhotosqpdf.py}{Inserción de fotos usando \qpdf\label{scr:pdfPhotosqpdf}}
	...
	\pythonScript{pdfPhotosAcrobat.py}{Inserción de fotos usando \acrobat\label{scr:pdfPhotosAcrobat}}
	...
	\begin{figure}
		...
		\caption[\texttt{PDF Optimizer} de \acrobat.]{\menu{PDF Optimizer} de \acrobat. Menú \menu{Images}\label{fig:pdfOptimizeImages}}
	\end{figure}
	...
	\begin{figure}
		...
		\caption[\texttt{Preflight} de \acrobat]{\menu{Preflight} de \acrobat. Pestaña \menu{Standards}\label{fig:pdfPdfaPreflightStandards}}
	\end{figure}
	...
	\begin{figure}
		\subcaptionbox{\menu{lossless}}{\includegraphics[width=\linewidth]{djvuSmallProfilesLossless}}
		\subcaptionbox{\menu{quasilossless}}{\includegraphics[width=\linewidth]{djvuSmallProfilesQuasilossless}}
		\caption[Diferencia entre compresión \texttt{lossless} y \texttt{quasilossless}]{Diferencia entre niveles de compresión \menu{lossless} y \menu{quasilossless} en el \djvu final\label{fig:djvuSmallProfilesQuality}}
	\end{figure}
	...
	Para comprimirlo abrimos \djvusmall (fig.~\ref{fig:djvuSmallBody}) y con \keys{Open File(s)} agregamos las imágenes del cuerpo. Seleccionamos luego el perfil \menu{MyBitonal} del menú desplegable \menu{Select encoding profile}. Elegimos el directorio de salida y presionamos \keys{Convert}. Al archivo resultante le llamaremos \typesetFile{BODY.djvu}.
	...
	\subsection{Unión de cuerpo y cubiertas} Para unir el cuerpo y las cubiertas ejecutamos el script~\ref{scr:djvuBundle}, que invoca al programa \typesetExe{djvm.exe} de \djvulibre. Este lo que hace es insertar \typesetFile{BODY.djvu} en \typesetFile{COVERS.djvu} a partir de la página \menu{startPage}, que en este caso es la \menu{2}.

	\pythonScript{djvuBundle.py}{Unión del cuerpo con las cubiertas de \djvu\label{scr:djvuBundle}}
	...
	\begin{itemize}[noitemsep]
		...
		\item botones \keys{Insert in DjVu}, para insertar las fotos, \keys{Options} para configurar algunas opciones y \keys{Help} para abrir el archivo de ayuda; y
	...
	\pythonScript{djvuSeparatePhotos.py}{Separación de las fotos creadas con \djvusmall\label{scr:djvuSeparatePhotos}}
	...
	\begin{figure}
		...
		\caption[Pestaña \texttt{Segmenter} de las opciones de \djvusmallmod]{Pestaña \menu{Segmenter} de las opciones de \djvusmallmod\label{fig:djvuSmallModSegmenterTab}}
	\end{figure}
	...
	\begin{figure}
		...
		\caption[Pestaña \texttt{Background} de las opciones de \djvusmallmod]{Pestaña \menu{Background} de las opciones de \djvusmallmod\label{fig:djvuSmallModBackgroundTab}}
	\end{figure}
	...
	\begin{figure}
		...
		\caption[Input file(s) de \djvusmallmod]{\menu{Input file(s)} de \djvusmallmod con imágenes bitonales y sus fotos correspondientes\label{fig:djvuSmallModListBackground}}
	\end{figure}
	...
	\section{Inserción de la capa \texorpdfstring{\ocr}{OCR}\label{sec:djvuOCR}} Para insertar la capa de texto que reconocimos tenemos el script~\ref{scr:djvuOCR}, que mediante la ejecución del programa \typesetExe{djvused.exe} de \djvulibre extrae la capa de texto del archivo \typesetFile{OCR.djvu} que creamos con \abbyy. Luego de la búsqueda y sustitución de algunas expresiones regulares, usa el mismo programa para insertarla en el documento \typesetFile{SCAN\_INS\_OCR.djvu}.

	\pythonScript{djvuOCR.py}{Inserción de la capa \ocr en documento \djvu\label{scr:djvuOCR}}
	...
	\pythonScript{djvuBookmarks.py}{Inserción de marcadores en documento \djvu\label{scr:djvuBookmarks}}
	...
	\pythonScript{djvuPageLabels.py}{Modificación de etiquetas de páginas en documento \djvu\label{scr:djvuPageLabels}}
	...
	\pythonScript{djvuMetadata.py}{Inserción de metadatos en documento \djvu\label{scr:djvuMetadata}}
...
% ------------------------------------------------------------------------------
	...
	\begin{figure}
		\subcaptionbox{Original}{\includegraphics[width=\linewidth]{SToutputPictureZonesGrayscale}}
		\subcaptionbox{Bitonal (Positioned)}{\includegraphics[width=\linewidth]{SToutputPictureZonesBitonal}}
	...
	De proceder así, todas nuestras imágenes serán bitonales (a excepción tal vez de las cubiertas). Así las cargaremos en \abbyy y luego podremos codificarlas en \djvu usando el perfil \menu{MyBitonal} sin discriminación.
	...
	\begin{figure}
		\subcaptionbox{Original}{\includegraphics[width=\linewidth]{SToutputPictureZonesGrayscale}}
		\subcaptionbox{\jpeg}{\includegraphics[width=\linewidth]{SToutputPictureZonesJpeg}}
	...
	En el caso del \pdf, tendrá mucho peso la optimización (sec.~\ref{sec:pdfOptimize}). Recomiendo experimentar con las diferentes opciones del menú \menu{Images}.
	...
	\begin{figure}
		...
		\caption[\texttt{Edit Settings} del formato \tiff en \acrobat]{\menu{Edit Settings} del formato \tiff en \acrobat\label{fig:simpleOCRpdfSettings}}
	\end{figure}
	...
	Ahora vamos a \menu{File > Create > Combine Files\...} y presionando en \keys{Options} aparece un cuadro (fig.~\ref{fig:simpleOCRpdfCombineOptions}) en el que desactivamos todas las casillas, presionamos \menu{File Size > \keys{Default File Size}}\footnote{Como se explica en el tooltip que se despliega al pasar con el ratón por encima, las opciones estarán controladas por el panel de preferencias, que acabamos de configurar.} y luego \keys{OK}.

	\begin{figure}
		\centering
		\includegraphics[width=.7\linewidth]{simpleOCRpdfCombineOptions}
		\caption[\texttt{Combine Files into a Single PDF\...} en \acrobat]{\menu{Combine Files into a Single PDF\... > Options} en \acrobat\label{fig:simpleOCRpdfCombineOptions}}
	\end{figure}
	...
	\begin{figure}
		...
		\caption[\texttt{Enhance Scanned PDF >\ Settings} en \acrobat]{\menu{Enhance Scanned PDF > Settings} en \acrobat\label{fig:simpleOCRpdfEnhanceSettings}}
	\end{figure}

	En \menu{Optimization Options} sugiero usar \menu{\typesetCheckbox{Apply Adaptive Compression}}. Esta opción hace una segmentación automática similar a del formato \djvu ya explicada. Luego tenemos opciones de compresión para imágenes que se explican a continuación.
	...
	En \menu{Filters} recomiendo desactivar todas las correcciones, poniéndolas en \menu{Off}. Análogamente al trabajo con \abbyy, ya nos encargamos de preparar el documento con \scantailor, por lo que no queremos ahora que se corrija más nada (y menos automáticamente y sin posibilidad de revisión).
	...
	\begin{figure}
		...
		\caption[\texttt{Text Recognition Options >\ Edit} en \acrobat]{\menu{Text Recognition Options > Edit} en \acrobat\label{fig:simpleOCRpdfEnhanceSettingsOCR}}
	\end{figure}

	La primera es equivalente a la opción \menu{Text under the page image} de \abbyy. Las imágenes se comprimen con las \menu{Optimization Options} elegidas y la capa de texto se inserta de manera invisible por debajo de estas.
	...
	\begin{figure}
		\subcaptionbox{Texto original}{\includegraphics[width=\linewidth]{simpleOCRpdfEnhanceGoodOriginal}}
		\subcaptionbox{Texto después de ClearScan}{\includegraphics[width=\linewidth]{simpleOCRpdfEnhanceGoodClearScan}}
		\caption[\texttt{Editable Text and Images/ClearScan} en \acrobat]{Resultado de la opción \menu{Editable Text and Images}/ClearScan sobre texto reconocido\label{fig:simpleOCRpdfEnhanceGoodExamples}}
	\end{figure}
...
\chapter{Historial de cambios}
	...
	\begin{itemize}
		...
		\item En «Optimización» (sec.~\ref{sec:pdfOptimize}) se explicaron con más detalle las opciones elegidas para \menu{Downsample} y \menu{Compression} del menú \menu{Images} y se actualizó la fig.~\ref{fig:pdfOptimizeImages}.
		...
		\begin{itemize}[noitemsep]
			\item Modificada la línea que invoca a \imagemagick (sec.~\ref{sec:pdfPhotosImageMagick}): afecta a \texttt{pdfPhotosqpdf.py} y \texttt{pdfPhotosAcrobat.py} (scr.~\ref{scr:pdfPhotosqpdf} y \ref{scr:pdfPhotosAcrobat}). Por lo tanto, ahora \imagemagick hace un \emph{resample} a 300\,dpi y luego aplica la compresión \jpeg deseada. Agregada una nota al pie explicando las razones.
			\item Agregada la opción \menu{bg-subsample=2} en el perfil \menu{MyPhoto} y cambiada su descripción (sec.~\ref{sec:profiles}).
			\item Actualizada «Codificación de las cubiertas» (sec.~\ref{sec:djvuSmallCovers}) advirtiendo que tal vez para las cubiertas sea mejor no hacer \emph{resample}.
			\item Actualizada «Inserción de fotos: DjVu Imager» (sec.~\ref{sec:djvuImager}) y la fig.~\ref{fig:djvuImager}, donde se habla de la opción \menu{BSF}.
			\item Actualizada «Codificación con DjVu Small Mod» (sec.~\ref{sec:djvuSmallMod}), donde se habla de \menu{Bg subsample}.
		\end{itemize}
		\item En «Codificación del cuerpo + inserción de fotos» (sec.~\ref{sec:djvuSmallModPhotos}) se cambió el método de codificación a \menu{Glue}. Ahora que lo entiendo un poco mejor parece el más apropiado. Se actualizó la fig.~\ref{fig:djvuSmallModSegmenterTab}.
		\item Aclarado en una nota al pie (sec.~\ref{sec:djvuPageLabels}) que para que funcione \texttt{paginate.py} (scr.~\ref{scr:djvuPageLabels}) los nombres de página que lleven algún espacio deben estar entre comillas dobles.
		\item Invertido el orden de «Creación de hipervínculos» (sec.~\ref{sec:djvuHyperlinks}) y «Metadatos» (sec.~\ref{sec:djvuMetadata}) porque \djvueditor suprime los metadatos si se usa después. Actualizado levemente \texttt{djvuMetadata.py} (scr.~\ref{scr:djvuMetadata}) para reflejar esto.
		\item Declarada la inclusión de material distinto del Gregory (ap.~\ref{sec:aboutGregory}).
		\item Mencionada la existencia de archivos ocultos en la estructura de \abbyy (ap.~\ref{sec:abbyyProjectStructure}).
		\item Creada la sección «Software» en la bibliografía.

		\item Otros muchos cambios y correcciones menores.
	\end{itemize}

	\section{v1.0 -- 2019-04-16}
	\begin{itemize}
