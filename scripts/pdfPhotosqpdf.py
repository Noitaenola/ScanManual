"""
Paste the separated photos in SCAN.pdf using qpdf.
"""
...
# Create a new SCAN_INS.pdf to work with (eventually overwriting the old
# one).
...
for filename in namesList:
    ...
    os.system(
        ...
        + backgroundDir
        + filename
        + " -transparent white -resample 300 -compress jpeg -quality 40 "
        + backgroundDir
        + filename
        + ".pdf"
    )
    ...
    # Call qpdf to paste that pdf in the corresponding page of SCAN_INS,
    # but save it as TEMP (can't specify the same file as both the input
    # and the output, thus the need for TEMP).
    os.system(
        "qpdf --overlay "
        + backgroundDir
        + filename
