"""
Generate an Adobe JavaScript script to later be run with Acrobat.
"""
...
with open(insertPhotos, "w") as outputFile:
    for filename in namesList:
        # Call ImageMagick to create a transparent-white pdf of the page.
        os.system(
            "magick convert "
            + backgroundDir
            + filename
            + " -transparent white -resample 300 -compress jpeg"
            + " -quality 40 "
            + backgroundDir
            + filename
            + ".pdf"
        ...
        print(
            'this.addWatermarkFromFile({cDIPath: "/C/Project/SCAN/out/'
            "background/" + filename + '.pdf", nSourcePage: 0, nStart:',
            str(int(re.search(r"\d+", filename).group(0)) - 1) + "});",
            file=outputFile,
        )
